...
?>
                <?php if($user_id<337){?>
                    <div class="alert alert-primary mt-3">
                            <strong>Your Semester has been automatically updated  by the System </strong>you can see the past semester saved results in <a href='../view_saved_results'>view saved results</a> section.
                    </div>
                <?php } ?>
                    ...
                    <div class="row mx-4 mb-2 py-2 px-3 warn2" style="font-weight:350; font-size:130%; display: none; background-color:rgba(255,213,0,0.8);color:#d00;">
                        Entered E-mail already exists
                    </div>
                                ...
                                <?php 
                                    for($i=0; $i<7; $i++)
                                    {
                                        if($i<3 && $i!=1 && $i!=2){
                                            ...
                                            echo "<tr><td><strong>$detail_name_array[$i]</strong></td>
                                            <td><span class='changeSpan{$i}'>$detail_array[$i]</span><input class='changeName{$i} form-control' name='$i' type='email' value='$detail_array[$i]' style='display: none;'></td>";
                                            echo "<td class='change{$i}' style='cursor:pointer;'>EDIT</td></tr>";
                                            echo "<script>document.querySelector('.change{$i}').addEventListener('click', function() {
                                                document.querySelector('.changeName{$i}').style.display = 'inline-block';
                                                document.querySelector('.changeSpan{$i}').style.display = 'none';
    ...
    if(isset($_GET['email']) && ($_GET['email'] == 'exists')){
        echo "<script>
            document.querySelector('.warn2').style.display = 'inline-block';
        </script>";
    }
    ?>
    <?php 
        if(isset($_POST['submit'])){
            ...
            if($form_array[2] != ""){
                if(!username_exists($form_array[2])){
                    $query .= "username = '$form_array[2]', ";
                    $_SESSION['username'] = $form_array[2];
                }
                else{
                   if($_SESSION['username'] != $form_array[2])
            ...
            if(!$username_already_exists)
              ...
              header('location:../profile/index.php?email=exists');
        }
    ?>
